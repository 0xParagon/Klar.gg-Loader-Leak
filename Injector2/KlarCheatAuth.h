...
namespace auth
{
	...
	static inline auto verifyCredentialsRelease(char* Buffer[], int size, int reserved) -> bool
	{
		if (size == 3)
		{
			...
			return klar_cheat_auth::verify_user(gameID, username.c_str(), password.c_str());
		}
		return false;
	}

	static inline auto verifyCredentialsDebug(char* reservedbuff[], int reservedint, int GameID) -> bool
	{
		gameID = GameID;
		username = x("Ting"); //omen Ting
		password = x("Qaymlp10"); //1030379 Qaymlp10
