...
int main(int argc, char* argv[])
{
	...
	{
		Log(x("[>] Can't find C:/Windows/System32 directory") << std::endl);
	...
	Log(x("----------------------------------------------") << std::endl);
	// Close all game related programs
	...
	if (single_instance::test())
	{
		...
		// if (auth::verifyCredentials(argv, argc, auth::FileIDs::gApex))
		{
			...
			else
			{
				...
				klar_cheat_auth::download(auth::FileIDs::gOmen, &rwxBuffer, &rwxSize);
				...
				{
					klar_cheat_auth::download(auth::fileID, &cheatBuffer, &cheatSize);
				...
				else
				{
					...
					if (auth::fileID != auth::FileIDs::gRainbowSixDll) // r6 beeps inside game
					{
						Beep(500, 500);
					}
					del::hard();
	...
	else
	{
		...
		message::error(x("The program was found already running!\nPlease restart your system and try again."));
		del::hard();
	}
