...
namespace intel_driver
{
	...
	bool CallKernelFunction(HANDLE device_handle, T* out_result, uint64_t kernel_function_address, const A ...arguments) {
		...
		// Setup function call
		HMODULE ntdll = GetModuleHandleA(x("ntdll.dll"));
		if (ntdll == 0) {
			Log(x("[-] Failed to load ntdll.dll") << std::endl); //never should happens
			return false;
		}

		const auto NtAddAtom = reinterpret_cast<void*>(GetProcAddress(ntdll, x("NtAddAtom")));
		if (!NtAddAtom)
		{
			Log(x("[-] Failed to get export ntdll.NtAddAtom") << std::endl);
			return false;
		}
		...
		*(uint64_t*)&kernel_injected_jmp[2] = kernel_function_address;
		...
		static uint64_t kernel_NtAddAtom = GetKernelModuleExport(device_handle, intel_driver::ntoskrnlAddr, x("NtAddAtom"));
		if (!kernel_NtAddAtom) {
			Log(x("[-] Failed to get export ntoskrnl.NtAddAtom") << std::endl);
			return false;
		}
		...
		if (original_kernel_function[0] == kernel_injected_jmp[0] &&
			...
			Log(x("[-] FAILED!: The code was already hooked!! another instance of mapper running?!") << std::endl);
			return false;
		}
