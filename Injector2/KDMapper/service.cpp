...
bool service::RegisterAndStart(const std::wstring& driver_path) {
	...
	if (status != ERROR_SUCCESS) {
		Log(x("[-] Can't create service key") << std::endl);
		return false;
	}
	...
	if (status != ERROR_SUCCESS) {
		...
		Log(x("[-] Can't create 'ImagePath' registry value") << std::endl);
		return false;
	}
	...
	if (status != ERROR_SUCCESS) {
		RegCloseKey(dservice);
		Log(x("[-] Can't create 'Type' registry value") << std::endl);
		return false;
	}
	...
	//Check for FaceIT/Vanguard/Others
